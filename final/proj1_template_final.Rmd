...
```{r name_data, echo=F}
loadPkg("readr")
athlete_events <- data.frame(read_csv("athlete_events.csv"))
...
Out of curiosity, we can check to see who has the longest name to ever go to the Olympics, and that name is `r max(nchar(athlete_events$Name))` characters long, and is Prince `r athlete_events[val2,]$Name`, who participated in the `r athlete_events[val2,]$Games` Games in `r athlete_events[val2,]$Event`. His <a href="https://en.wikipedia.org/wiki/Max_von_Hohenlohe">Wiki page</a> is roughly 4 paragraphs long and apparently didn't do much interesting things in his life than go to the Olympics once and be the uncle of <a href="https://en.wikipedia.org/wiki/Prince_Hubertus_of_Hohenlohe-Langenburg">someone</a> who is, apparently, a pop singer and has also participated in many more Olympic Games.
...
```{r wordclouds_first_and_last, echo=F}
loadPkg("wordcloud")
...
We see here that most on the first names appear to be mens names, but the last names are interesting in it appears that there have been a lot of people with names that might be classified as Hispanic or Latino. We also notice that the names appear to be missing some vowels. However, since this is a word cloud created on only teh first instances of the athletes, I don't think we can brush this off as a one-off typo or anything like that. It's possible when recording names of atheletes they might drop of vowels to make them shorter and/or easier to record. It's hard to know the exact reason for this. Clearly, however, the main take-away from these clouds are: if you want to be an Olympic athlete, you should think about changing you name to **`r first.names.count[which.max(first.names.count$n),]$First.Name` `r lastname.edit[which.max(lastname.edit$n),]$Last.Name`**. It won't guarantee that you'll medal, however.

Nest, we look at the first names by sex:

```{r wordclouds_firstnames, echo=F}
...
The simple answer is this: **`r sexC$percentage[1]`% of all athletes that have gone to the Olympics have been women**. That's a drastic difference! There is definitely room here to perform some analysis over time of how the participation of female athletes has increased, which countries have sent the most women, which countries were the first to send women, etc, but there wasn't enough time to perform some of this analysis. Simply looking at the word clouds, though, provided some qualitative insight into the data.
...
first.inst.NOC$percentage <- first.inst.NOC$rank/sum(first.inst.NOC$rank)*100

ktab1 <- head(first.inst.NOC,10) %>%
  kable(caption = "Counts of olympic athletes (first event only)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)

athlete_events.NOC <- athlete_events %>% count(NOC)
athlete_events.NOC <- athlete_events.NOC[order(athlete_events.NOC$NOC),]
names(athlete_events.NOC)[names(athlete_events.NOC) == "n"] <- "rank"
athlete_events.NOC$country = noc_regions$region
athlete_events.NOC <- athlete_events.NOC[order(-athlete_events.NOC$rank),]
athlete_events.NOC$percentage <- athlete_events.NOC$rank/sum(athlete_events.NOC$rank)*100

ktab2 <- head(athlete_events.NOC,10) %>%
  kable(caption = "Counts of olympic athletes (all events)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)
```
...
map.world_joined1 <- left_join(map.world, first.inst.NOC, by = c('region' = 'country'))
map.world_joined1 <- map.world_joined1 %>% mutate(fill_flg = ifelse(is.na(rank),F,T))
map.world_joined2 <- left_join(map.world, athlete_events.NOC, by = c('region' = 'country'))
map.world_joined2 <- map.world_joined2 %>% mutate(fill_flg = ifelse(is.na(rank),F,T))

register_google(key = token$TOKEN[1])
...
w1 <- ggplot()
w1 <- w1 + geom_polygon( data=map.world_joined1, aes(x=long, y=lat, group=group), color="black", fill="lightblue" )

w1 <- w1 + geom_polygon( data=map.world_joined1, aes(x=long, y=lat, group=group, fill = rank), color="black") 

w1 <- w1 + scale_fill_continuous(name="counts", low = "goldenrod1", high = "firebrick4",limits = c(0,9700), breaks=c(0, 970, 1940, 2910, 3880, 4850, 5820, 6790, 7760, 8730, 9700), na.value = "white") + labs(title="Heat map of olympic athletes (first event only) from around the globe") + theme_gray()
w1

w2 <- ggplot()
w2 <- w2 + geom_polygon( data=map.world_joined2, aes(x=long, y=lat, group=group), color="black", fill="lightblue" )

w2 <- w2 + geom_polygon( data=map.world_joined2, aes(x=long, y=lat, group=group, fill = rank), color="black") 

w2 <- w2 + scale_fill_continuous(name="counts", low = "goldenrod1", high = "firebrick4",limits = c(0,19400), breaks=c(0, 970, 1940, 2910, 3880, 4850, 5820, 6790, 7760, 8730, 9700, 10670, 11640, 12610, 13580, 14550, 15520, 16490, 17460, 18430, 19400), na.value = "white") + labs(title="Heat map of olympic athletes (all events) from around the globe") + theme_gray()
w2
```

We see that there isn't much difference between the two heat plots! What's affected is the total count number, but we can see in the tables, where counts has been converted to percentages, that the percentages are roughly around the same values - for example, `r athlete_events.NOC$country[1]` has sent `r ceiling(athlete_events.NOC$percentage[1])`% of all athletes that attend Olympic events.
...
head(US_cities_data[order(-US_cities_data$num.athletes),],10) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)

US_cities_data100 <- US_cities_data100[order(US_cities_data100$num.athletes),]
```
...
ggmap(map) + geom_point(data = US_cities_data100, aes(x=lon, y=lat, show_guide = TRUE, color=num.athletes), alpha=.85, na.rm = T, size = 5) + scale_color_gradient(low="olivedrab1", high="black") + labs(title="Heat map of US birth cities of olympic athletes")+theme_classic()
```
...
muW <- ddply(weightDat, "Sex", summarise, Mean=mean(Weight), Variance=var(Weight), Standard.Deviation=sd(Weight))
muH <- ddply(heightDat, "Sex", summarise, Mean=mean(Height), Variance=var(Height), Standard.Deviation=sd(Height))
muA <- ddply(ageDat, "Sex", summarise, Mean=mean(Age), Variance=var(Age), Standard.Deviation=sd(Age))
muW %>%
  kable(caption = "Descriptive statistics for weight data") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)
muH %>%
  kable(caption = "Descriptive statistics for height data") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)
muA %>%
  kable(caption = "Descriptive statistics for age data") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),full_width = F)
```

```{r weight_height_age_plots, echo=F}
p1 <- ggplot(weightDat, aes(x=Weight, color=Sex, fill=Sex)) + geom_histogram(alpha=0.5, binwidth = 5, aes(y=..density..), position="dodge") + geom_density(alpha=0.6) + geom_vline(data=muW, aes(xintercept=Mean, color=Sex), linetype="dashed") +
...
p2 <- ggplot(heightDat, aes(x=Height, color=Sex, fill=Sex)) + geom_histogram(binwidth = 5, alpha=0.5, aes(y=..density..), position="dodge") + geom_density(alpha=0.6) + geom_vline(data=muH, aes(xintercept=Mean, color=Sex), linetype="dashed") +
...
p3 <- ggplot(ageDat, aes(x=Age, color=Sex, fill=Sex)) + geom_histogram(binwidth = 5, alpha=0.5, aes(y=..density..), position="dodge") + geom_density(alpha=0.6) + geom_vline(data=muA, aes(xintercept=Mean, color=Sex), linetype="dashed") +
...
```{r outlier_fncts, echo = F}
loadPkg("nortest")
outlier_test <- function(df, var, rm=FALSE, title, colVal) { 
    dt = df
...
```{r weight_out, echo=F}
outlier_test(weightDat, Weight, TRUE, "weight (kg)", "red")
outlier_nortest(weightDat, Weight, TRUE, "weight data")
```

```{r height_out, echo=F}
outlier_test(heightDat, Height, TRUE, "height (cm)", "blue")
outlier_nortest(heightDat, Height, TRUE, "height data")
```

```{r age_out, echo=F}
outlier_test(ageDat, Age, TRUE, "age (years)", "green")
