...
sex_survival_plot <- ggplot(freq_sex_total, aes(x=reorder(sex.cond, -rel.freq.adj), y=rel.freq.adj, 
                           fill=sex.cond)) +
...
class_survive_plot <- ggplot(freq_pclass_tot, aes(x=reorder(condition, -adj.freq), y=adj.freq, 
                           fill=condition)) +
...
We can add `sex` into the model as a predictor. 

```{r logitmodel_age_pclass_sex}
surviveLogit_age_pclass_sex <- glm(survived ~ age + pclass + sex, data = titanic_orig, family = "binomial")
summary(surviveLogit_age_pclass_sex)
```

All the coefficients are found significant (small p-values). We see that `age`, `pclass2` (2nd class), `pclass3`, and `sexmale` all negatively affect the likelihood of survival. These are reasonable results and confirms the results found above.  

We can also obtain the growth/decay factors for each explanatory variables as we did in the previous section.  

```{r growthDecayFactors_age_pclass_sex}
expcoeff_age_pclass_sex = exp(coef(surviveLogit_age_pclass_sex))
expcoeff_age_pclass_sex
```

We remember that any value below 1 represents a negative effect. From these results, we can say:

* The log(odds-ratio) for being admitted improve by a factor of `r expcoeff_age_pclass_sex[2]` for each point of `age` score increase. Notice that for a, say 20-point age improvement, the chance of survival is changed not 20 times that value. Instead, the log(odds-ratio) improved by a factor of $(`r expcoeff_age_pclass_sex[2]`)^{20}$.  

* The improvement of `pclass2` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[3]`.

* The improvement of `pclass3` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[4]`. 

* The improvement of `sexmale` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[5]`.

We can use the Hosmer and Lemeshow Goodness of Fit test to evaluate logistic regression fit. 

```{r HosmerLemeshow_age_pclass_sex}
admitLogitHoslem_age_pclass_sex <- hoslem.test(titanic_orig$survived, fitted(surviveLogit_age_pclass_sex)) 
admitLogitHoslem_age_pclass_sex
```

The Hosmer-Lemeshow statistic indicates a poor fit if the significance value is less than 0.05. The p-value of `r admitLogitHoslem_age_pclass_sex$p.value` is below 0.05 which indicates the model is not really a good fit, despite the fact that all the coefficients are significant. 

Receiver-Operator-Characteristic (ROC) curve and Area-Under-Curve (AUC) measures the true positive rate (or sensitivity) against the false positive rate (or specificity). The area-under-curve is always between 0.5 and 1. Values higher than 0.8 is considered good model fit. 

```{r roc_auc_age_pclass_sex}
prob_age_pclass_sex <- predict(surviveLogit_age_pclass_sex, type = c("response"))
titanic_orig$prob <- prob_age_pclass_sex
h_age_pclass_sex <- roc(survived~prob, data=titanic_orig)
auc(h_age_pclass_sex) # area-under-curve prefer 0.8 or higher.
plot(h_age_pclass_sex)
```

We have here the area-under-curve of `r auc(h_age_pclass_sex)`, which is larger than 0.8. This test does not agrees with the Hosmer and Lemeshow test that the model is not considered a good fit, because values larger than 0.8 indicate a good fit. We can use another test to see how good our model is.  

McFadden is another evaluation tool we can use on logit regressions. This is part of what is called pseudo-R-squared values for evaluation tests. We can calculate the value directly like we did before.

```{r McFadden_direct_age_pclass_sex}
admitNullLogit_age_pclass_sex <- glm(survived ~ 1, data = titanic_orig, family = "binomial")
mcFadden_age_pclass_sex <- 1 - logLik(surviveLogit_age_pclass_sex)/logLik(admitNullLogit_age_pclass_sex)
mcFadden_age_pclass_sex
```

With the McFadden value of `r mcFadden_age_pclass_sex`, which is analgous to the coefficient of determination $R^2$, about 33% of the variations in y is explained by the explanatory variables in the model. 
...
We want to calculate a predicted probability of survival, for specific values of the predictors: 

1. sex is female, age is 10, and she is a second class passenger.
2. sex is male, age is 20, and he is a first class passenger

To do that, we create a new data frame, in which we include the desired values for our prediction.

```{r predict_female_10_2ndclass_survive}
# prob of 1
newdata <- data.frame(age = 10, sex = "female", pclass = "2", survived = "1")
probpredict_f10_2ndclass_live <- predict(surviveLogit_age_pclass_sex, newdata, type="response")
# prob of 2
newdata1 <- data.frame(age = 20, sex = "male", pclass = "1", survived = "1")
probpredict_m20_1stclass_live <- predict(surviveLogit_age_pclass_sex, newdata1, type="response")
``` 

From our second model, the probability of a passenger surviving if she is a young girl that is 10 years old and in 2nd class is `r 100*probpredict_f10_2ndclass_live`%. 

The probability of a passenger surviving if he is a young adult man of 20 years and in 1st class is `r 100*probpredict_m20_1stclass_live`%. 
...
```{r hist_age_survival}
survived_df_0s <- na.omit(survived_df_0s)
survived_df_1s <- na.omit(survived_df_1s)

hist_ages_lived <- ggplot(survived_df_1s, aes(x=age)) +
   geom_histogram(aes(y=..density..), binwidth = 3, alpha = 0.35, 
                  position="identity", fill="lightblue", color="blue") + 
   theme_minimal() +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
   labs(title="Histogram of \nthe ages of the passengers \nthat survived", x="age (in years)", y="relative frequency") + 
  geom_vline(xintercept = mean(survived_df_1s[["age"]]), linetype="dotted", 
             color = "purple", size=1.5) 

hist_ages_died <- ggplot(survived_df_0s, aes(x=age)) +
   geom_histogram(aes(y=..density..), binwidth = 3, alpha = 0.35, 
                  position="identity", fill="pink", color="red") + 
   theme_minimal() +
   theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
   labs(title="Histogram of \nthe ages of the passengers \nthat did not survived", x="age (in years)", y="relative frequency") + 
  geom_vline(xintercept = mean(survived_df_0s[["age"]]), linetype="dotted", 
             color = "darkred", size=1.5) 

grid.arrange(hist_ages_lived, hist_ages_died, ncol = 2, nrow = 1)
```

From the histograms and the ANOVA test we know that the mean ages between the groups that survived and did not survive are different. We see that the ages for the group that survived skew slightly younger, and we can see how the ages for those that survived have a peak around ages that we would associated with babies. We also see that essentially no middle ages individuals survived, and that only the oldest individuals (around 80) survived. This result is agreed with by the logistic regression results, which showed that age negatively affects survival (or, from the last model, we saw that the log(odds-ratio) for being admitted improve by a factor of `r expcoeff_age_pclass_sex[2]` for each point of `age` score increase---the value below 1 means that it's a negative effect). 

This confirms our general belief that children and the elderly would be allowed to escape first.   

## Summary of Sex Results
...
The logistic regression analysis confirms the traditional analysis we did in the first part. It does indeed seem that age, sex, and class affected the chances of survival for the passengers aboard the Titanic. Although there were less women (`r 100*tot_num_fm$rel.freq[tot_num_fm$sex == "female"]`% of passengers) if you were a woman you had a `r 100*freq_sex_total$rel.freq.adj[freq_sex_total$sex.cond == "female.lived"]`% chance of living. This is opposed to men, who were the majority (`r 100*tot_num_fm$rel.freq[tot_num_fm$sex == "male"]`% of passengers), but if you were a man you had a `r 100*freq_sex_total$rel.freq.adj[freq_sex_total$sex.cond == "male.died"][1]`% chance of dying. The chi-square test shows us that the chances of survival is NOT independent from the sex. We also see from the last model that being a male negatively affects your chances of survival (we saw that the improvement of `sexmale` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[5]`). 

This confirms our belief that women were allowed to escape first, before men. 

## Summary of Class Results

```{r class_survive_show}
class_survive_plot
```

While first class passengers were only `r 100*(tot1st/nrow(titanic_orig))`% of the total passengers, you had a `r 100*freq_pclass_tot$adj.freq[freq_pclass_tot$condition == "first.lived"]`% chance of living if you were first class. This is opposed to third class passengers, who were `r 100*(tot3rd/nrow(titanic_orig))`% of the total passengers, but if you were third class then you had `r 100*freq_pclass_tot$adj.freq[freq_pclass_tot$condition == "third.died"]`% chance of dying. Second class passengers had roughly a 50-50 chance of survival. This shows that if you were a wealthier passenger that you were saved over poorer passengers. This result is confirmed by the logistic regression models as well. According the the last model, if you were a second class passenger the improvement of `pclass2` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[3]`. If you were a third class passenger the improvement of `pclass3` of 1 point will increase the log(odds-ratio) by a factor of `r expcoeff_age_pclass_sex[4]`.

We see that wealthier passengers in first class were more likely to survive than the poored passengers. 

## Overall

We see that if you were very young or very old, you were likely to survive the sinking of the Titanic. If you were a woman you were also likely to survive. If you were a wealthier passenger in first class you were also likely to survive. This means that the group most likely to survive the sinking of the titanic would be a young woman in first class. We can find the probability for this as well. We will choose a few hypothetical individuals to examine: 

1. a young 10 year old girl in first class
3. a 20 year old woman in first class
2. a man of average age (~30 for both groups) in third class

```{r predict_survival}
newdata2 <- data.frame(age = 10, sex = "female", pclass = "1", survived = "1")
prob_newdata2 <- predict(surviveLogit_age_pclass_sex, newdata2, type="response")

newdata3 <- data.frame(age = 30, sex = "male", pclass = "3", survived = "1")
prob_newdata3 <- predict(surviveLogit_age_pclass_sex, newdata3, type="response")

newdata4 <- data.frame(age = 20, sex = "female", pclass = "1", survived = "1")
prob_newdata4 <- predict(surviveLogit_age_pclass_sex, newdata4, type="response")
```

The probability of surviving the sinking of the Titanic, according to our last model, if you are a young 10 year old girl in first class was `r 100*prob_newdata2`. A 20 year old woman in first class had a `r 100*prob_newdata4`% chance of survival. This is compared to the probability of surviving if you were a 30 year old man in third class, which was `r 100*prob_newdata3`%.

Here we can see it plainly: in order to survive the sinking of the Titanic you needed to be a child, a woman, and a wealthier passenger in first class. 
